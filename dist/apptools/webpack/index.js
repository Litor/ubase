...
function generatorEntryFiles(path, webpack, userConfig, entrys) {
  ...
  appPathList.forEach(function (appPath) {
    ...
    var indexHtmlFilePath = relativePath(path.resolve(_config2.default.src) + '/pages/' + appName + '/index.html');
    var configFilePath = relativePath(path.resolve(_config2.default.src) + '/pages/' + appName + '/config.json');
    ...
    var i18nImport = '';
    var indexHtmlImport = 'require("' + indexHtmlFilePath + '")';
    var configImport = '';

    // 框架代码 引用路径
    var ubasePath = _config2.default.isProduction ? '../../ubase' : '../../ubase';

    var useConfigFile = 'false';

    var useI18n = 'false';

    if (appI18nFilesPath.length > 0 || globalI18nFilesPath.length > 0) {
      useI18n = 'true';
      i18nImport = generateI18nImport(appName);
    }

    if (userConfig.useConfigFile) {
      useConfigFile = 'true';
      configImport = 'require("' + configFilePath + '")';
    }
    ...
    var fileContent = templateReplace(appEntryTemplate, {
      ...
      ubase: { content: ubasePath, relativePath: false, required: true },
      ...
      useConfigFile: { content: useConfigFile, relativePath: false, required: true, statement: true }, // 是否需要config.json文件
      useI18n: { content: useI18n, relativePath: false, required: true, statement: true }, // 是否需要国际化
      routes: { content: routeFilePath, relativePath: true, required: true },
      indexHtmlImport: { content: indexHtmlImport, relativePath: false, required: true, statement: true },
      configImport: { content: configImport, relativePath: false, required: true, statement: true },
