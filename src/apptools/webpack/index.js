...
function generatorEntryFiles(path, webpack, userConfig, entrys) {
  ...
  appPathList.forEach(function (appPath) {
    ...
    let indexHtmlFilePath = relativePath(path.resolve(config.src) + '/pages/' + appName + '/index.html')
    let configFilePath = relativePath(path.resolve(config.src) + '/pages/' + appName + '/config.json')
    ...
    // 框架代码 引用路径
    var ubasePath = config.isProduction ? '../../ubase' : '../../ubase'
